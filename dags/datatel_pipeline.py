...
#configurations - all environment variables are stored in Airflow Variables
...
#branch function - determines whether to run the full pipeline 
#or just the incremental load based on the presence of a 
# last processed timestamp in the watermark table
...
with DAG(
    ...
    #STAGE 1: freshness checks on billing & sessions only
    ...
    #STAGE 1: data quality checks. Load bad rows to quarantine table. Pipeline always continues
    ...
    #STAGE 2" Load staging tables; clean, cast, deduplicate.
    ...
    #branch operation to determine whether to run full or incremental load for customers
    ...
    #STAGE 3: watermarking - run after successful staging loads.
    ...
    #STAGE 4: load GCS CSV into BQ staging table
    ...
    #STAGE 4: BQ merge - upsert staging table into final table - dw_user_analytics table
    ...
    #STAGE 5: Truncate staging table - runs only after successful merge to final table
    truncate_staging = PostgresOperator(
        task_id = 'truncate_staging',
        postgres_conn_id = 'postgres_conn_id',
        sql = 'sql/warehouse/truncate_staging.sql',
    )
    ...
    # Freshness checks run in parallel — independent reads
    # No dependencies, they start immediately when the DAG runs
    [check_freshness_billings, check_freshness_sessions]

    # All quality checks wait for BOTH freshness checks
    # Because each quality check file covers multiple source tables
    ...
    # All staging loads wait for ALL quality checks
    [check_completeness, check_uniqueness,
    check_referential_integrity, check_validity] >> load_stg_billings

    [check_completeness, check_uniqueness,
    check_referential_integrity, check_validity] >> load_stg_sessions

    [check_completeness, check_uniqueness,
    check_referential_integrity, check_validity] >> branch_customers

    # Branch picks one customer load, skips the other
    branch_customers >> [load_stg_customers_incremental, load_stg_customers_full]

    # Watermarks update after their respective staging loads only
    load_stg_billings  >> update_watermark_billing
    load_stg_sessions >> update_watermark_sessions

    # Aggregations wait for all staging + watermarks
    # NONE_SKIPPED handles the skipped branch path
    [update_watermark_billing, update_watermark_sessions,
    load_stg_customers_incremental, load_stg_customers_full] >> load_agg_user_revenue

    [update_watermark_billing, update_watermark_sessions,
    load_stg_customers_incremental, load_stg_customers_full] >> load_agg_user_usage

    [update_watermark_billing, update_watermark_sessions,
    load_stg_customers_incremental, load_stg_customers_full] >> load_agg_monthly_revenue

    [update_watermark_billing, update_watermark_sessions,
    load_stg_customers_incremental, load_stg_customers_full] >> load_session_buckets

    # These two wait for their direct upstream dependency only
    load_agg_monthly_revenue >> load_agg_arpu
    load_session_buckets     >> load_agg_session_distribution

    # Warehouse export waits for all six aggregations
    [load_agg_user_revenue, load_agg_user_usage,
    load_agg_arpu, load_agg_session_distribution] >> export_to_gcs

    # GCS → BQ staging → MERGE → truncate staging
